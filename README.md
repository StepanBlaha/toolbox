...
A collection of **65+ design & developer utilities** - CSS generators, color tools, image tools, and IT utilities. Fast, free, and **fully client-side**: nothing you paste, upload, or generate ever leaves your browser.
...
Browse the full grid on the landing page, or use the **search bar** (searches names, descriptions, and common synonyms, so "colour", "encrypt", or "rounded" all find the right tool).

**CSS & layout** - Box Shadow · Text Shadow · Text Stroke · Gradient · Gradient Text · Gradient Border · Grainy Gradient · Mesh Gradient · Noise Texture · Pattern · Glassmorphism · Neumorphism · Clip-path · Border Radius · Scrollbar · CSS Animation (keyframes) · CSS Loaders · 3D Transform · Shape · Blob · SVG Waves · CSS Filters · SVG to CSS · Bento Grid (drag & drop)

**Color & type** - Contrast Checker · Color Converter (HEX/RGB/HSL/OKLCH) · Color Palette Generator · Palette Extractor · Design Tokens · Font Pairing (138 Google fonts) · Type Scale

**Image** - Background Remover (in-browser ML) · Image Effects (13 effects: halftone, glitch, low-poly, dither & more) · Dither & Retro · Image to ASCII · Duotone · Image Compressor · Masked Text · Favicon Generator · OG Image Generator · Now Playing Card

**Dev & IT utilities** - JSON Formatter · Base64 · Hash (SHA-1/256/384/512) · JWT Decoder · AES Encrypt (AES-GCM) · UUID · Password Generator · Regex Tester · Diff Checker · Cron Helper · Timestamp Converter · Base Converter · Case Converter · URL Tools · Unit Converter · Meta Tag Generator · Text Stats · Lorem Ipsum · ASCII Banner · QR Code · QR with Logo
...
npm run build    # tsc + vite build + per-route SEO prerender into dist/
...
src/
  ...
  pages/landing/          Tool grid + search
...
The search, sitemap, per-route SEO prerender, and landing grid all derive from `data/tools.ts` automatically.
